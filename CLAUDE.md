...
- **그림자 효과 규칙**: 자연스러운 그림자 효과를 위해 최상위 뷰에 `android:clipToPadding="false"` 속성을 설정하여 그림자가 패딩 영역까지 표시되도록 해야 함. `cardElevation`, `translationZ`, `elevation` 속성을 조합하여 깊이감 표현. 패딩과 마진을 적절히 조합하여 레이아웃 구성
